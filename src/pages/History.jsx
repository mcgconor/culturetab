...
export default function History({ session: propSession, logTrigger }) {
  ...
  useEffect(() => {
    ...
    else supabase.auth.getSession().then(({ data }) => { if (data?.session?.user) fetchHistory(data.session.user.id); else setLoading(false); });
  }, [propSession]);
  ...
  useEffect(() => {
    ...
    if (filters.kind !== 'all') result = result.filter(e => e.kind === filters.kind);
    if (filters.date) result = result.filter(e => e.event_date === filters.date);
    if (filters.rating !== 'all') { const min = Number(filters.rating); result = result.filter(e => e.rating >= min); }
  ...
  const fetchHistory = async (userId) => {
    try {
        setLoading(true);
        const { data, error } = await supabase.from('entries').select('*').eq('user_id', userId).order('event_date', { ascending: false });
        if (error) throw error;
        setAllEntries(data || []); setFilteredEntries(data || []);
  ...
  const handleEditTrigger = (item) => { setEntryToEdit(item); setShowEntryForm(true); };

  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this memory?")) return;
    const { error } = await supabase.from('entries').delete().eq('id', id);
    if (!error) { const remaining = allEntries.filter(item => item.id !== id); setAllEntries(remaining); }
  };
    ...
    <div className="animate-fade-in relative">
      ...
      <div className="px-4">
        {loading ? <div className="space-y-4">{[1,2,3].map(i => <div key={i} className="h-32 bg-gray-100 rounded-xl animate-pulse"></div>)}</div> :
         filteredEntries.length === 0 ? <div className="text-center py-20 bg-gray-50 rounded-2xl border border-gray-100"><p className="text-gray-500 font-medium">No results match your filters.</p><button onClick={() => setFilters({search: '', kind: 'all', date: '', rating: 'all'})} className="mt-4 text-black font-bold underline text-sm">Clear Filters</button></div> :
         <div className="space-y-4">{filteredEntries.map(item => <UniversalCard key={item.id} item={item} type="entry" onAction={handleEditTrigger} onDelete={handleDelete} />)}</div>}
      </div>
